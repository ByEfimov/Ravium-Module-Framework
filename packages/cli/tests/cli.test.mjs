...
test('ravium module init creates swiper carousel module scaffold', async () => {
  const workspace = await mkdtemp(path.join(tmpdir(), 'ravium-cli-swiper-'));
  ...
  const moduleDir = path.join(workspace, 'swiper-carousel');
  const manifest = JSON.parse(await readFile(path.join(moduleDir, 'ravium.module.json'), 'utf8'));
  const packageJson = JSON.parse(await readFile(path.join(moduleDir, 'package.json'), 'utf8'));

  assert.equal(manifest.namespace, 'ravium');
  assert.equal(manifest.slug, 'swiper-carousel');
  assert.equal(packageJson.dependencies['@ravium/module-sdk'], '^1.0.0');
  assert.equal(manifest.description, 'Swiper.js carousel component for Ravium applications.');
  assert.equal(manifest.settingsSchema.project.properties.paginationEnabled.type, 'boolean');
  assert.deepEqual(manifest.settingsSchema.project.properties.styleMode.enum, ['swiper', 'ravium']);
  assert.equal(manifest.settingsSchema.carousel.properties.autoplayDelayMs.type, 'number');
  assert.deepEqual(manifest.settingsSchema.carousel.properties.direction.enum, ['horizontal', 'vertical']);
  assert.equal(manifest.settingsSchema.carousel.properties.slides.type, 'array');
  assert.equal(manifest.settingsSchema.carousel.properties.a11yEnabled.type, 'boolean');
  assert.equal(manifest.settingsSchema.carousel.properties.virtualEnabled.type, 'boolean');
  assert.equal(manifest.settingsSchema.carousel.properties.parallaxEnabled.type, 'boolean');
  assert.equal(manifest.settingsSchema.carousel.properties.hashNavigationEnabled.type, 'boolean');
  assert.equal(manifest.settingsSchema.carousel.properties.historyEnabled.type, 'boolean');
  assert.equal(manifest.settingsSchema.carousel.properties.effectRotate.type, 'number');
  assert.equal(manifest.components[0].type, 'swiper.carousel');
  assert.equal(manifest.components[0].runtimeRenderer, 'src/components/SwiperCarousel.vue');
  assert.equal(manifest.components[0].editorRenderer, 'src/editor-renderer.html');
  assert.equal(manifest.components[0].editorPreview, undefined);
  assert.ok(manifest.components[0].editorStyleDeclarations.length > 0);
  assert.ok(manifest.variables.some((variable) => variable.key === 'swiperCarouselSlideGap'));
  assert.equal(manifest.dependencies.runtime.swiper, '^12.2.0');
  await stat(path.join(moduleDir, 'src/components/SwiperCarousel.vue'));
  await stat(path.join(moduleDir, 'src/editor-renderer.html'));

  const runtimeSource = await readFile(path.join(moduleDir, 'src/components/SwiperCarousel.vue'), 'utf8');
  assert.match(runtimeSource, /const modules = computed\(\(\) =>/);
  assert.match(runtimeSource, /if \(autoplayConfig\.value\) \{\n    active\.push\(Autoplay\);/);
  assert.match(runtimeSource, /if \(paginationConfig\.value\) \{\n    active\.push\(Pagination\);/);
  assert.match(runtimeSource, /:virtual="virtualConfig"/);
  assert.match(runtimeSource, /:parallax="parallaxEnabled"/);
  assert.match(runtimeSource, /:hash-navigation="hashNavigationConfig"/);
  assert.match(runtimeSource, /:history="historyConfig"/);
  assert.match(runtimeSource, /<slot/);
  assert.match(runtimeSource, /componentSlotName\(slide, index\)/);
  assert.doesNotMatch(runtimeSource, /Controller/);
  assert.doesNotMatch(runtimeSource, /Manipulation/);
  assert.doesNotMatch(runtimeSource, /Thumbs/);
  assert.doesNotMatch(runtimeSource, /import 'swiper\/css\/navigation'/);
  assert.doesNotMatch(runtimeSource, /import 'swiper\/css\/pagination'/);
  assert.doesNotMatch(runtimeSource, /import 'swiper\/css\/thumbs'/);
  assert.match(runtimeSource, /import 'swiper\/css';/);
  assert.doesNotMatch(runtimeSource, /componentRegistry/);
  assert.doesNotMatch(runtimeSource, /componentTarget\(slide\)/);

  const editorRendererSource = await readFile(path.join(moduleDir, 'src/editor-renderer.html'), 'utf8');
  assert.match(editorRendererSource, /scheduleSlotPublishing/);
  assert.match(editorRendererSource, /slideChange: scheduleSlotPublishing/);
});
